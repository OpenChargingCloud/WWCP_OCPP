...
namespace cloud.charging.open.protocols.OCPPv1_6.CS
{
                                         ...
                                         ICentralSystem
    {
                                                                ...
                                                                CancellationToken          CancellationToken)
        {
            ...
            else if (Connection.HTTPRequest?.SecWebSocketProtocol.Contains(Version.WebSocketSubProtocolId) == false)
            {

                DebugX.Log($"This web socket service only supports '{Version.WebSocketSubProtocolId}'!");
                ...
                return Task.FromResult<HTTPResponse?>(
                           new HTTPResponse.Builder() {
                               ...
                               Content         = JSONObject.Create(
                                                     new JProperty("description",
                                                         JSONObject.Create(
                                                             new JProperty("en", $"This web socket service only supports '{Version.WebSocketSubProtocolId}'!")
                                                     ))).ToUTF8Bytes(),
                               Connection      = "close"
                           }.AsImmutable);
                                                           ...
                                                           CancellationToken          CancellationToken)
        {
                 ...
                 Connection.HTTPRequest is not null)
            {
                ...
                if (Connection.HTTPRequest.Authorization is HTTPBasicAuthentication httpBasicAuthentication)
                {
                    ...
                    if (ChargeBox_Id.TryParse(httpBasicAuthentication.Username, out chargeBoxId))
                    {
                        ...
                        if (!connectedChargeBoxes.TryGetValue(chargeBoxId, out var value))
                            connectedChargeBoxes.TryAdd(chargeBoxId, new Tuple<WebSocketServerConnection, DateTime>(Connection, Timestamp.Now));
                        ...
                        else
                        {
                            ...
                            connectedChargeBoxes.TryRemove(chargeBoxId, out _);
                            connectedChargeBoxes.TryAdd   (chargeBoxId, new Tuple<WebSocketServerConnection, DateTime>(Connection, Timestamp.Now));

                            try
                            {
                                oldChargeBox_WebSocketConnection.Close();
                            }
                            catch (Exception e)
                            {
                                await HandleErrors(
                ...
                else if (ChargeBox_Id.TryParse(Connection.HTTPRequest.Path.ToString()[(Connection.HTTPRequest.Path.ToString().LastIndexOf("/") + 1)..], out chargeBoxId))
                {
                    ...
                    if (!connectedChargeBoxes.TryGetValue(chargeBoxId, out Tuple<WebSocketServerConnection, DateTime>? value))
                         connectedChargeBoxes.TryAdd(chargeBoxId, new Tuple<WebSocketServerConnection, DateTime>(Connection, Timestamp.Now));
            ...
            #region Send OnCentralSystemNewWebSocketConnection event

            var logger = OnCentralSystemNewWebSocketConnection;
                                                 ...
                                                 CancellationToken                 CancellationToken)
        {
            ...
            if (Connection.TryGetCustomDataAs<ChargeBox_Id>(chargeBoxId_WebSocketKey, out var chargeBoxId))
            {

                connectedChargeBoxes.TryRemove(chargeBoxId, out _);

                #region Send OnCentralSystemCloseMessageReceived event

                var logger = OnCentralSystemCloseMessageReceived;
                if (logger is not null)
                {
                         ...
                         json[4].Type == JTokenType.Object)
                {
                    ...
                    lock (requests)
                    {
                            ...
                            requests.TryGetValue(requestId, out var request))
                        {
                            ...
                            // ToDo: Refactor 
                            if (ResultCode.TryParse(json[2]?.Value<String>() ?? "", out var errorCode))
                                request.ErrorCode = errorCode;
                            else
                                request.ErrorCode = ResultCode.GenericError;
        ...
        //        {
        //            sendRequestState.ErrorCode = ResultCode.Timeout;
        //            requests.TryRemove(RequestId, out _);
        ...
        //        var webSocketConnections  = WebSocketConnections.Where  (ws => ws.TryGetCustomDataAs<ChargeBox_Id>(chargeBoxId_WebSocketKey) == ChargeBoxId).
        //                                                         ToArray();
        ...
        #region SendJSONData     (EventTrackingId, ChargeBoxId, NetworkPath, RequestId, Action, JSONData,   RequestTimeout, ...)

        /// <summary>
        /// Send (and forget) the given JSON.
        /// </summary>
        /// <param name="EventTrackingId">An event tracking identification for correlating this request with other events.</param>
        /// <param name="ChargeBoxId">The networking node identification of the message destination.</param>
                                                               ...
                                                               CancellationToken  CancellationToken   = default)
        {
            ...
            try
            {
                ...
                var webSocketConnections  = WebSocketConnections.Where  (connection => connection.TryGetCustomDataAs<ChargeBox_Id>(chargeBoxId_WebSocketKey) == ChargeBoxId).
                                                                 ToArray();
