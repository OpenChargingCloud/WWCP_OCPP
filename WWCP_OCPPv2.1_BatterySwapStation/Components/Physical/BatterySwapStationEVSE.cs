...
namespace cloud.charging.open.protocols.OCPPv2_1.BSS
{
    ...
    /// <summary>
    /// An EVSE at a battery swap station.
    /// </summary>
    public class BatterySwapStationEVSE : EVSESpec0
    {

        #region Data

        private   readonly ABatterySwapStationNode                                          parentChargingStation;

        protected readonly ConcurrentDictionary<Connector_Id, BatterySwapStationConnector>  connectors         = [];
        ...
        internal BatterySwapStationEVSE(ABatterySwapStationNode         ChargingStation,
                                     EVSE_Id                      Id,
        ...
        public IEnumerable<BatterySwapStationConnector> Connectors
            => connectors.Values;

        public Boolean TryGetConnector(Connector_Id ConnectorId, [NotNullWhen(true)] out BatterySwapStationConnector? Connector)
            => connectors.TryGetValue(ConnectorId, out Connector);


        public BatterySwapStationConnector? AddConnector(ConnectorType ConnectorType)
        {

            lock (connectors)
            {

                var connector = new BatterySwapStationConnector(
                                    this,
        ...
        public async Task DoMaintenanceAsync(ABatterySwapStationNode  ChargingStation,
                                             Object?               State)
        {
