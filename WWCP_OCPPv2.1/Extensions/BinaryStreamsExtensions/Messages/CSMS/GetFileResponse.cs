                                   ...
                                   IResponse
    {
                                       ...
                                       CustomBinaryParserDelegate<GetFileResponse>?  CustomGetFileResponseParser   = null)
        {
            ...
            try
            {
                ...
                var fileConentLength      = stream.ReadUInt64();
                var fileConent            = stream.ReadBytes(fileConentLength);

                #endregion

                #region File content type

                var fileConentTypeLength  = stream.ReadByte();
                if (fileConentTypeLength < 0)
                {
                    ErrorResponse = "The received file content type length is invalid!";
                    return false;
                }
                var fileConentType        = ContentType.TryParse(stream.ReadBytes((UInt64) fileConentTypeLength)?.ToUTF8String() ?? "");
