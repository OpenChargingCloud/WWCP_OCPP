...
namespace cloud.charging.open.protocols.OCPPv2_1.NetworkingNode
{
    ...
    public partial class OCPPWebSocketAdapterFORWARD
    {
        ...
        public event OnNotifyDisplayMessagesRequestFilterDelegate?      OnNotifyDisplayMessagesRequest;

        public event OnNotifyDisplayMessagesRequestFilteredDelegate?    OnNotifyDisplayMessagesRequestLogging;

        #endregion

        public async Task<ForwardingDecision<NotifyDisplayMessagesRequest, NotifyDisplayMessagesResponse>>

            Forward_NotifyDisplayMessages(NotifyDisplayMessagesRequest  Request,
...

        {
            ...
            var requestFilter = OnNotifyDisplayMessagesRequest;
            if (requestFilter is not null)
            {
                try
                {
                    ...
                    forwardingDecision = response.Result == ForwardingResult.DROP && response.DropResponse is null
                                             ? new ForwardingDecision<NotifyDisplayMessagesRequest, NotifyDisplayMessagesResponse>(
                                                   response.Request,
            ...
            var resultLog = OnNotifyDisplayMessagesRequestLogging;
            if (resultLog is not null)
            {
                ...
                catch (Exception e)
                {
                    await HandleErrors(
                              nameof(TestNetworkingNode),
                              nameof(OnNotifyDisplayMessagesRequestLogging),
                              e
                          );
                }
