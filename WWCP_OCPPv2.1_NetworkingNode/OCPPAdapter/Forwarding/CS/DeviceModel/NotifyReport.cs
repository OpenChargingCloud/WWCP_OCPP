...
namespace cloud.charging.open.protocols.OCPPv2_1.NetworkingNode
{
    ...
    public partial class OCPPWebSocketAdapterFORWARD
    {
        ...
        public event OnNotifyReportRequestFilterDelegate?      OnNotifyReportRequest;

        public event OnNotifyReportRequestFilteredDelegate?    OnNotifyReportRequestLogging;

        #endregion

        public async Task<ForwardingDecision<NotifyReportRequest, NotifyReportResponse>>

            Forward_NotifyReport(NotifyReportRequest   Request,
...

        {
            ...
            var requestFilter = OnNotifyReportRequest;
            if (requestFilter is not null)
            {
                try
                {
                    ...
                    forwardingDecision = response.Result == ForwardingResult.DROP && response.DropResponse is null
                                             ? new ForwardingDecision<NotifyReportRequest, NotifyReportResponse>(
                                                   response.Request,
            ...
            var resultLog = OnNotifyReportRequestLogging;
            if (resultLog is not null)
            {
                ...
                catch (Exception e)
                {
                    await HandleErrors(
                              nameof(TestNetworkingNode),
                              nameof(OnNotifyReportRequestLogging),
                              e
                          );
                }
