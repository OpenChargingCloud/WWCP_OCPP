...
namespace cloud.charging.open.protocols.OCPPv2_1.NetworkingNode
{
    ...
    public partial class OCPPWebSocketAdapterFORWARD
    {
        ...
        public event OnStatusNotificationRequestFilterDelegate?      OnStatusNotificationRequest;

        public event OnStatusNotificationRequestFilteredDelegate?    OnStatusNotificationRequestLogging;

        #endregion

        public async Task<ForwardingDecision<StatusNotificationRequest, StatusNotificationResponse>>

            Forward_StatusNotification(StatusNotificationRequest  Request,
...

        {
            ...
            var requestFilter = OnStatusNotificationRequest;
            if (requestFilter is not null)
            {
                try
                {
                    ...
                    forwardingDecision = response.Result == ForwardingResult.REJECT && response.RejectResponse is null
                                             ? new ForwardingDecision<StatusNotificationRequest, StatusNotificationResponse>(
                                                   response.Request,
            ...
            var resultLog = OnStatusNotificationRequestLogging;
            if (resultLog is not null)
            {
                ...
                catch (Exception e)
                {
                    await HandleErrors(
                              nameof(TestNetworkingNode),
                              nameof(OnStatusNotificationRequestLogging),
                              e
                          );
                }
