...
namespace cloud.charging.open.protocols.OCPPv2_1.NetworkingNode
{
    ...
    public partial class OCPPWebSocketAdapterFORWARD
    {
        ...
        public event OnClearDisplayMessageRequestFilterDelegate?      OnClearDisplayMessageRequest;

        public event OnClearDisplayMessageRequestFilteredDelegate?    OnClearDisplayMessageRequestLogging;
...

        {
            ...
            var requestFilter = OnClearDisplayMessageRequest;
            if (requestFilter is not null)
            {
                try
                {
                    ...
                    forwardingDecision = response.Result == ForwardingResult.REJECT && response.RejectResponse is null
                                             ? new ForwardingDecision<ClearDisplayMessageRequest, ClearDisplayMessageResponse>(
                                                   response.Request,
            ...
            var resultLog = OnClearDisplayMessageRequestLogging;
            if (resultLog is not null)
            {
                ...
                catch (Exception e)
                {
                    await HandleErrors(
                              nameof(TestNetworkingNode),
                              nameof(OnClearDisplayMessageRequestLogging),
                              e
                          );
                }
