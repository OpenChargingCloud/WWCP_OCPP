...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    public class GetBaseReportCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                                  ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(GetBaseReportCommand)[..^7].ToLowerFirstChar();
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length >= 2)
            {
                ...
                if (!ReportBase.TryParse(Arguments[1], out var reportBase))
                    return [ $"Invalid report base: '{Arguments[1]}'!" ];

                var response = await cli.OCPP.OUT.GetBaseReport(
                                         new GetBaseReportRequest(
                                             Destination:              sourceRoute,
                                             GetBaseReportRequestId:   (Int32) RandomExtensions.RandomUInt32(Int32.MaxValue),
            ...
            return [ $"Usage: {CommandName} [{ReportBase.All.Select(_ => _.ToString()).AggregateWith("|")}]" ];
        ...
        public override String Help()
            => $"{CommandName} [{ReportBase.All.Select(_ => _.ToString()).AggregateWith("|")}] - Get a base report";
