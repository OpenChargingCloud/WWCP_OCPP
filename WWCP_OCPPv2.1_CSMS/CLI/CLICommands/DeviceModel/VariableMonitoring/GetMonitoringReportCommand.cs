...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    public class GetMonitoringReportCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                                            ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(GetMonitoringReportCommand)[..^7].ToLowerFirstChar();
        ...
        public override IEnumerable<SuggestionResponse> Suggest(String[] Arguments)
        {
                ...
                CommandName.Equals(Arguments[0], StringComparison.OrdinalIgnoreCase))
            {
                ...
                foreach (var monitoringCriterion in MonitoringCriteriaExtensions.All)
                {

                    if (monitoringCriterion.ToString().Equals    (Arguments[1], StringComparison.OrdinalIgnoreCase))
                        return [ SuggestionResponse.ParameterCompleted($"{Arguments[0]} {monitoringCriterion.ToString().ToLower()}") ];

                    if (monitoringCriterion.ToString().StartsWith(Arguments[1], StringComparison.OrdinalIgnoreCase))
                        return [ SuggestionResponse.ParameterPrefix   ($"{Arguments[0]} {monitoringCriterion.ToString().ToLower()}") ];
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length >= 3)
            {
                ...
                if (!MonitoringCriteriaExtensions.TryParse(Arguments[1], out var monitoringCriterion))
                    return [ $"Invalid monitoring criterion: '{Arguments[1]}'!" ];

                var response = await cli.OCPP.OUT.GetMonitoringReport(
                                         new GetMonitoringReportRequest(
                                             Destination:                    sourceRoute,
                                             GetMonitoringReportRequestId:   (Int32) RandomExtensions.RandomUInt32(Int32.MaxValue),
            ...
            return [ $"Usage: {CommandName} <{MonitoringCriteriaExtensions.All.Select(_ => _.ToString()).AggregateWith("|")}> <ComponentName> [VariableName]" ];
        ...
        public override String Help()
            => $"{CommandName} <{MonitoringCriteriaExtensions.All.Select(_ => _.ToString()).AggregateWith("|")}> <ComponentName> [VariableName] - Get a base report";
