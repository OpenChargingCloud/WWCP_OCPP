...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    public class ClearVariableMonitoringCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                                                ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(ClearVariableMonitoringCommand)[..^7].ToLowerFirstChar();
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length >= 2)
            {
                ...
                var response = await cli.OCPP.OUT.ClearVariableMonitoring(
                                         new ClearVariableMonitoringRequest(
                                             Destination:             sourceRoute,
                                             VariableMonitoringIds:   variableMonitoringIds
                                         )
                                     );
            ...
            return [ $"Usage: {CommandName} <variableMonitoringId> [variableMonitoringId ...]" ];
        ...
        public override String Help()
            => $"{CommandName} <variableMonitoringId> [variableMonitoringId ...] - Clear a variable monitoring by its identification";
