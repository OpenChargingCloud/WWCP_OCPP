...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    public class SetMonitoringBaseCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                                          ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(SetMonitoringBaseCommand)[..^7].ToLowerFirstChar();
        ...
        public override IEnumerable<SuggestionResponse> Suggest(String[] Arguments)
        {
                ...
                CommandName.Equals(Arguments[0], StringComparison.OrdinalIgnoreCase))
            {
                ...
                foreach (var monitoringBase in MonitoringBase.All2)
                {

                    if (monitoringBase.ToString().Equals    (Arguments[1], StringComparison.OrdinalIgnoreCase))
                        return [ SuggestionResponse.ParameterCompleted($"{Arguments[0]} {monitoringBase.ToString().ToLower()}") ];

                    if (monitoringBase.ToString().StartsWith(Arguments[1], StringComparison.OrdinalIgnoreCase))
                        return [ SuggestionResponse.ParameterPrefix   ($"{Arguments[0]} {monitoringBase.ToString().ToLower()}") ];
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length >= 2)
            {
                ...
                if (!MonitoringBase.TryParse(Arguments[1], out var monitoringBase))
                    return [ $"Invalid monitoring base '{Arguments[1]}'" ];

                var response = await cli.OCPP.OUT.SetMonitoringBase(
            ...
            return [ $"Usage: {CommandName} <{MonitoringBase.All2.Select(_ => _.ToString()).AggregateWith("|")}>" ];
        ...
        public override String Help()
            => $"{CommandName} <{MonitoringBase.All2.Select(_ => _.ToString()).AggregateWith("|")}> - Set the monitoring base";
