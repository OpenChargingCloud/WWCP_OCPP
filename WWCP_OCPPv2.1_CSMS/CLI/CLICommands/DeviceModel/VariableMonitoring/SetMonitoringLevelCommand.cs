...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    public class SetMonitoringLevelCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                                           ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(SetMonitoringLevelCommand)[..^7].ToLowerFirstChar();
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length >= 2)
            {
                ...
                if (!SeveritiesExtensions.TryParse(Arguments[1], out var severity))
                    return [ $"Invalid severity '{Arguments[1]}'" ];

                var response = await cli.OCPP.OUT.SetMonitoringLevel(
            ...
            return [ $"Usage: {CommandName} <{SeveritiesExtensions.All.Select(_ => _.ToString()).AggregateWith("|")}>" ];
        ...
        public override String Help()
            => $"{CommandName} <{SeveritiesExtensions.All.Select(_ => _.ToString()).AggregateWith("|")}> - Set the monitoring level";
