...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    public class GetReportCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                                  ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(GetReportCommand)[..^7].ToLowerFirstChar();
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length >= 1)
            {
                ...
                if (Arguments.Length >= 2)
                {
                    ...
                    if (ComponentCriteriaExtensions.TryParse(Arguments[1], out var _componentCriteria))
                        componentCriteria = _componentCriteria;
                    else
                        return [ $"Invalid component criteria: '{Arguments[1]}'!" ];
                ...
                var response = await cli.OCPP.OUT.GetReport(
                                         new GetReportRequest(
                                             Destination:          sourceRoute,
                                             GetReportRequestId:   (Int32) RandomExtensions.RandomUInt32(Int32.MaxValue),
                                             ComponentCriteria:    componentCriteria.HasValue
                                                                       ? new List<ComponentCriteria>() { componentCriteria.Value }
                                                                       : null,
                                             ComponentVariables:   null   // ComponentVariable
                                         )
                                     );
            ...
            return [ $"Usage: {CommandName} [{ComponentCriteriaExtensions.All.Select(_ => _.ToString()).AggregateWith("|")}]" ];
        ...
        public override String Help()
            => $"{CommandName} [{ComponentCriteriaExtensions.All.Select(_ => _.ToString()).AggregateWith("|")}] - Get a report";
