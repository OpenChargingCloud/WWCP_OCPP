...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    /// <summary>
    /// Get the local list version number
    /// </summary>
    /// <param name="CLI">The command line interface</param>
    //[CLIContext([ DefaultStrings.OCPPv2_0_1,
    //              DefaultStrings.OCPPv2_1 ])]
    public class GetLocalListCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                                     ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(GetLocalListCommand)[..^7].ToLowerFirstChar();
        ...
        public override IEnumerable<SuggestionResponse> Suggest(String[] Arguments)
        {
            ...
            //    //foreach (var resetType in GetLocalListType.All)
            //    //{
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length == 1)
            {
                ...
                var response = await cli.OCPP.OUT.GetLocalListVersion(
        ...
        public override String Help()
            => $"{CommandName} - Get the local auth list of the current networking node";
