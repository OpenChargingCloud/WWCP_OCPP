...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    public class ClearCacheCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                                     ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(ClearCacheCommand)[..^7].ToLowerFirstChar();
        ...
        public override IEnumerable<SuggestionResponse> Suggest(String[] Arguments)
        {
            ...
            //    //foreach (var resetType in ClearCacheType.All)
            //    //{
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length == 1)
            {
                ...
                var response = await cli.OCPP.OUT.ClearCache(
        ...
        public override String Help()
            => $"{CommandName} - Clear the local auth cache of the current networking node";
