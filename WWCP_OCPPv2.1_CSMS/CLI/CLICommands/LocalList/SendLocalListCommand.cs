...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    public class SendLocalListCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                              ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(SendLocalListCommand)[..^7].ToLowerFirstChar();
        ...
        public override IEnumerable<SuggestionResponse> Suggest(String[] Arguments)
        {
            ...
            //    //foreach (var resetType in SendLocalListType.All)
            //    //{
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length >= 4)
            {
                ...
                if (!UInt32.TryParse(Arguments[1], out var versionNumber))
                    return [$"Invalid version number '{Arguments[1]}'!"];

                if (!UpdateTypesExtensions.TryParse(Arguments[2], out var updateType))
                    return [$"Invalid update type '{Arguments[2]}'!"];

                var authorizationList = new List<AuthorizationData>();

                foreach (var token in Arguments.Skip(3))
                {
                    if (IdToken.TryParseRFID(token, out var idToken))
                ...
                var response = await cli.OCPP.OUT.SendLocalList(
                                         new SendLocalListRequest(
                                             Destination:             sourceRoute,
                                             VersionNumber:           versionNumber,
                                             UpdateType:              updateType,
                                             LocalAuthorizationList:  authorizationList
                                         )
                                     );
            ...
            return [ $"Usage: {CommandName} <VersionNumber> <full|differential> [aabbccdd, 11223344556677, ...]" ];
        ...
        public override String Help()
            => $"{CommandName} <VersionNumber> <full|differential> [aabbccdd, 11223344, ...] - SendLocalList the current networking node";
