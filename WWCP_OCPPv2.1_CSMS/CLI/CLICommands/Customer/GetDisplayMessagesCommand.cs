...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    public class GetDisplayMessagesCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                                           ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(GetDisplayMessagesCommand)[..^7].ToLowerFirstChar();
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length >= 1)
            {
                ...
                var displayMessageIds = new List<DisplayMessage_Id>();

                foreach (var displayMessageIdText in Arguments.Skip(1))
                {
                    if (DisplayMessage_Id.TryParse(displayMessageIdText, out var displayMessageId))
                        displayMessageIds.Add(displayMessageId);
                }

                var response = await cli.OCPP.OUT.GetDisplayMessages(
                                         new GetDisplayMessagesRequest(
                                             Destination:                   sourceRoute,
                                             GetDisplayMessagesRequestId:   (Int32) RandomExtensions.RandomUInt32(Int32.MaxValue),
                                             Ids:                           displayMessageIds.Count > 0
                                                                                ? displayMessageIds
                                                                                : null,
                                             Priority:                      null,  // MessagePriority
                                             State:                         null   // MessageState
                                         )
                                     );
            ...
            return [ $"Usage: {CommandName} [messageId1] ..." ];
        ...
        public override String Help()
            => $"{CommandName} [messageId1] ... - Get all display messages or only the ones specified by their ids";
