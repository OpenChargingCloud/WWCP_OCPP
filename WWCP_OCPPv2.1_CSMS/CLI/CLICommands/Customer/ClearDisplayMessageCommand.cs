...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    public class ClearDisplayMessageCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                                            ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(ClearDisplayMessageCommand)[..^7].ToLowerFirstChar();
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length >= 2)
            {
                ...
                if (!DisplayMessage_Id.TryParse(Arguments[1], out var displayMessageId))
                    return [ $"Invalid display message identification '{Arguments[1]}'" ];

                var response = await cli.OCPP.OUT.ClearDisplayMessage(
            ...
            return [ $"Usage: {CommandName} <display message id> ..." ];
        ...
        public override String Help()
            => $"{CommandName} <display message id> - Remove the specified display message";
