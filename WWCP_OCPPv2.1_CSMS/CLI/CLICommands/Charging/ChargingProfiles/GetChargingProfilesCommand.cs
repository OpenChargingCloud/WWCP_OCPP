...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    public class GetChargingProfilesCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                                           ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(GetChargingProfilesCommand)[..^7].ToLowerFirstChar();
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length >= 1)
            {
                ...
                if (Arguments.Length >= 2)
                {
                    if (EVSE_Id.TryParse(Arguments[1], out var _evseId))
                        evseId = _evseId;
                    else
                        return [ $"Invalid EVSE identification '{Arguments[1]}'" ];
                }

                var response = await cli.OCPP.OUT.GetChargingProfiles(
                                         new GetChargingProfilesRequest(
                                             Destination:                    sourceRoute,
                                             GetChargingProfilesRequestId:   (Int32) RandomExtensions.RandomUInt32(Int32.MaxValue),
                                             ChargingProfile:                new ChargingProfileCriterion(
                                                                                 ChargingProfilePurpose:   null,  // ChargingProfilePurpose
                                                                                 StackLevel:               null,  // UInt32
                                                                                 ChargingProfileIds:       null,  // IEnumerable<ChargingProfile_Id>
                                                                                 ChargingLimitSources:     null   // IEnumerable<ChargingLimitSource>
                                                                             ),
                                             EVSEId:                         evseId
                                         )
                                     );
        ...
        public override String Help()
            => $"{CommandName} [transactionId] - Show transaction status (for the given transaction identification)";
