...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    public class ClearChargingProfileCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                                           ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(ClearChargingProfileCommand)[..^7].ToLowerFirstChar();
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length >= 1)
            {
                ...
                if (Arguments.Length >= 2)
                {
                    if (ChargingProfile_Id.TryParse(Arguments[1], out var _chargingProfileId))
                        chargingProfileId = _chargingProfileId;
                    else
                        return [ $"Invalid charging profile identification '{Arguments[1]}'" ];
                }

                var response = await cli.OCPP.OUT.ClearChargingProfile(
                                         new ClearChargingProfileRequest(
                                             Destination:               sourceRoute,
                                             ChargingProfileId:         chargingProfileId,
                                             ChargingProfileCriteria:   null//new ClearChargingProfile(
                                                                        //    EVSEId:                   null,
            ...
            return [ $"Usage: {CommandName} [chargingProfileId]" ];
        ...
        public override String Help()
            => $"{CommandName} [chargingProfileId] - Remove a charging profile";
