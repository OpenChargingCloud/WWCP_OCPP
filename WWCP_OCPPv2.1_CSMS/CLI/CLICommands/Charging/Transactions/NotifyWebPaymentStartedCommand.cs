...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    public class NotifyWebPaymentStartedCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                                                ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(NotifyWebPaymentStartedCommand)[..^7].ToLowerFirstChar();
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length >= 1)
            {
                ...
                if (!UInt16.TryParse(Arguments[2], out var timeoutSeconds))
                    return [$"Invalid timeout seconds '{Arguments[2]}'"];

                var response = await cli.OCPP.OUT.NotifyWebPaymentStarted(
            ...
            return [ $"Usage: {CommandName} <evseId> <timeout seconds>" ];
        ...
        public override String Help()
            => $"{CommandName} <evseId> <timeout seconds> - Notify about a started web payment process";
