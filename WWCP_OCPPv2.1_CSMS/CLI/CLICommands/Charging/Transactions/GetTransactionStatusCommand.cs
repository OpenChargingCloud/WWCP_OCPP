...
namespace cloud.charging.open.protocols.OCPPv2_1.CSMS.CommandLine
{
    ...
    public class GetTransactionStatusCommand(ICSMSCLI CLI) : ACLICommand<ICSMSCLI>(CLI),
                                                             ICLICommand
    {

        #region Data

        public static readonly String CommandName = nameof(GetTransactionStatusCommand)[..^7].ToLowerFirstChar();
                                                     ...
                                                     CancellationToken  CancellationToken)
        {
            ...
            if (Arguments.Length >= 1)
            {
                ...
                if (Arguments.Length >= 2)
                {
                    if (Transaction_Id.TryParse(Arguments[1], out var _transactionId))
                        transactionId = _transactionId;
                    else
                        return [ $"Invalid transaction identification '{Arguments[1]}'" ];
                }

                var response = await cli.OCPP.OUT.GetTransactionStatus(
